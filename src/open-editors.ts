...
export async function getOpenFiles(workspaceRootPath: string): Promise<File[]> {
    ...
    const editors = openEditors
        ...
        .map(_ => {
            return new File(getRelativeFilepath(workspaceRootPath, _.document.uri.path));
        });

    return editors;
}

export function getTestFiles(workspaceRootPath: string): File[] {
    const testFiles = [
    ...
    return testFiles.map((file) => {
        return new File(getRelativeFilepath(workspaceRootPath, file));
    });
}
...
export class ActiveEditorTracker extends vscode.Disposable {
    ...
    private _resolver: ((value?: vscode.TextEditor | PromiseLike<vscode.TextEditor>) => void) | undefined;
